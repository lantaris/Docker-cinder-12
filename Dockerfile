...
LABEL maintainer="edik.ponomarenko@mail.ru" name="Cinder + keystone container is used for link Virt 4.1/4.2 (0.0.1) and SDS Ceph V12(luminous) RBD (based on CenOS 7)" vendor="Lantaris" license="GPLv2" build-date=""20181005"" version="0.0.1"

########################################################
#                        INSTALLING                    #
########################################################
RUN yum clean all && yum -y install epel-release &&yum -y update &&\
    yum -y install centos-release-openstack-pike yum-utils https://download.ceph.com/rpm-luminous/el7/noarch/ceph-release-1-1.el7.noarch.rpm &&\
